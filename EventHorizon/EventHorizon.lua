...
function ns:CheckRequirements()
	...
	for i,config in ipairs(self.frames.config) do
		...
		if haveGlyphReq and haveSpecReq and haveLevelReq and haveTalentReq and haveTalentRequiredUnselected then
			if frameExists then
				spellframe:Activate()
			else
...
function ns:Initialize()
    ...
    if not EventHorizonDB.__WodClassStatusNotification2 then
        StaticPopupDialogs["EH_Dialog1"] = {
            ...
            hideOnEscape = 1,
            OnAccept = function()
                StaticPopup_Hide("EH_Dialog1")
                EventHorizonDB.__WodClassStatusNotification2 = true
            end
        }
        StaticPopup_Show("EH_Dialog1")
    end
