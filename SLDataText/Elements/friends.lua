...
local function SetupToolTip()
	tool:SetScript("OnEnter", function(this)
		...
		for k, v in pairs(friendList) do
			...
			local lineL = string.format("%s |cff%s%s|r %s", lvl, cCol, name, status or "")
		...
		if ( BNetList and select("#", BNetList) >= 1 ) then
			for k, v in pairs(BNetList) do
				local BNid, BNname, battleTag, toonclass, toonname, client, status, broadcast, note = v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9]
				
				if ( toonclass and toonclass == "Death Knight" ) then toonclass = "DEATHKNIGHT" else toonclass = string.upper(toonclass) end
				local cCol
				
				if toonclass == "" then
					cCol = "ffffff"
				else
					cCol = string.format("%02X%02X%02X", RAID_CLASS_COLORS[toonclass].r*255, RAID_CLASS_COLORS[toonclass].g*255, RAID_CLASS_COLORS[toonclass].b*255)
				end
				
				local lineL = string.format("|cffecd672%s|r | |cffecd672%s|r (|cff%s%s|r)", BNname, battleTag, cCol, toonname)
...
function SLDT.Friends:Refresh()
	if ( db.enabled or SLDataText.db.profile.configMode ) then
		...
		for j = 1, BNGetNumFriends() do
			...
			if ( online ) then
				local _, _, _, _, _, _, race, toonclass, unknown, zoneName, level, gameText, broadcastText, broadcastTime = BNGetFriendToonInfo(j,  BNGetNumFriendToons(j))
				friendsOn = friendsOn + 1
				if (not battleTag) then battleTag = "[noTag]" end
				friendList = friendList or {}
				local status = ""
				if ( isafk ) then status = L["(AFK)"] end; if ( isdnd ) then status = L["(DND)"] end
				table.insert(BNetList, { BNid, BNname, battleTag, toonclass, toonname, client, status, broadcast, note })
			end
		end
