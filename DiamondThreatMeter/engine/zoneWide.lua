...
function DTM_ZoneWide_CanCheckRaidCombat()
    ...
    if ( DTM_GetSavedVariable("engine", "workMethod", "active") == "NATIVE" ) then
        return "DISABLED";
    end

    if ( not UnitAffectingCombat("player") ) then return "INVALID"; end
    if ( (GetNumGroupMembers() + GetNumGroupMembers()) == 0 ) then return "INVALID"; end
