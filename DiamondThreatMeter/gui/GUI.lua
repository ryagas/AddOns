...
elseif ( erased == 2 ) then
    ...
    DTM_GUI_OnSkinEvent("ANCHOR_INIT");
...
elseif not ( DTM_GUI_Shown ) and ( status == 1 ) then
    ...
    if ( GetTime() >= (DTM_Update["CHECK_PARTY"] or 0) ) then
        ...
        local inParty = ( GetNumGroupMembers() > 0 or GetNumGroupMembers() > 0 );
...
elseif ( event == "ANCHOR_INIT" ) then
        --  All elements of the GUI can now query their anchor.
        for unit, frame in pairs(DTM_ThreatListFrames) do frame:LoadVerticalAnchor(); end
        for unit, frame in pairs(DTM_PresenceListFrames) do frame:LoadVerticalAnchor(); end
        for unit, frame in pairs(DTM_RegainListFrames) do frame:LoadVerticalAnchor(); end
    end
end
...
function DTM_GUI_OnSelfAggro(name, guid, event, icon)
    ...
    if ( ( GetNumGroupMembers() + GetNumGroupMembers() ) > 0 and condition == "PARTY" ) then okay = true; end
