                        ...
                        if ( object.valueList ) then
                            ...
                            for i=1, #object.valueList do
                                if ( object.valueList[i] == settingValue ) then
                                    ...
                                    _G[object:GetName().."Text"]:SetText(DTM_Localise(object.stringList[i], true) or settingValue);
                                end
	                    end
                        end
                    end
...
function DTM_SkinEditor_SetupCategory(frame, name)
    ...
    for w=1, frame.numWidgets do
        ...
        if ( dataTable.object == "CHECKBOX" ) then
            ...
            _G[object:GetName().."Text"]:SetText(displayName);
            object:SetHitRectInsets(0, -_G[object:GetName().."Text"]:GetStringWidth(), 0, 0);
        end
        ...
        elseif ( step < 1 ) then
            ...
            _G[object:GetName().."Text"]:SetText(displayName);
            _G[object:GetName().."Low"]:SetText(lowBound);
            _G[object:GetName().."High"]:SetText(highBound);
        end
        ...
        if ( dataTable.object == "DROPDOWN" ) then
            ...
            _G[object:GetName().."Caption"]:SetText(displayName);
        end
        ...
        if ( dataTable.object == "COLUMN" ) then
            ...
            _G[object:GetName().."Title"]:SetText(displayName);
        end
    ...
    for w=1, frame.numWidgets do
        ...
        if type(object) == "table" then
            if ( object.family == "CHECKBOX" ) then
                ...
                curY = curY - 24;
                maxX = max(maxX, _G[object:GetName().."Text"]:GetStringWidth() + 64); -- Ensure we've got enough room on X-axis.
            end

            if ( object.family == "SLIDER" ) then
                object:SetPoint("TOPLEFT", frame, "TOPLEFT", 20, curY);
                curY = curY - 28;
                maxX = max(maxX, _G[object:GetName().."Text"]:GetStringWidth() + 192);
            end

            if ( object.family == "DROPDOWN" ) then
                object:SetPoint("TOPLEFT", frame, "TOPLEFT", 0, curY);
                curY = curY - 32;
                maxX = max(maxX, _G[object:GetName().."Caption"]:GetStringWidth() + object.width + 64);
            end

            if ( object.family == "EDITBOX" ) then
                object:SetPoint("TOPLEFT", frame, "TOPLEFT", 24, curY-6);
                curY = curY - 32;
                maxX = max(maxX, _G[object:GetName().."Text"]:GetStringWidth() + object.width + 52);
            end
    ...
    frame.titleText = _G[frame:GetName().."_TitleText"];
    frame.titleTexture = _G[frame:GetName().."_TitleTexture"];
...
function DTM_SkinEditor_ColumnEditor_OnLoad(self)
    self.enabled  = _G[self:GetName().."_EnabledCheckButton"];
    self.position = _G[self:GetName().."_PositionSlider"];
    self.justifyText   = _G[self:GetName().."Justification"];
    self.justifyLeft   = _G[self:GetName().."_JustifyLeft"];
    self.justifyCenter = _G[self:GetName().."_JustifyCenter"];
    self.justifyRight  = _G[self:GetName().."_JustifyRight"];

    _G[self:GetName().."Enabled"]:SetText(DTM_Localise("Enabled"));
    _G[self:GetName().."Position"]:SetText(DTM_Localise("Position"));
...
function DTM_SkinEditor_Slider_OnValueChanged(self)
    ...
    _G[self:GetName().."Value"]:SetText(newValueText);
