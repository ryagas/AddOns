...
function ChatThrottleLib:Init()	
	...
	if not self.aFrame then
		self.aFrame = CreateFrame("Frame")
		self.aFrame:Hide()
	end
	self.aFrame:SetScript("OnUpdate", self.OnUpdate)
	self.aFrame:SetScript("OnEvent", self.OnEvent)	-- v11: Monitor P_E_W so we can throttle hard for a few seconds
	self.aFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
...
if(WOWB_VER) then
	...
	ChatThrottleLib.aFrame:SetScript("OnEvent", SayTimer)
	ChatThrottleLib.aFrame:RegisterEvent("CHAT_MSG_SAY")
end
]]
