...
local NameplateLib = _G.CoolNameplateLib;
...
elseif ( type(NameplateLib) ~= "nil" and type(NameplateLib) ~= "table" ) then
    error("CoolNameplateLib could not be loaded. <CoolNameplateLib> global namespace was already used by something else.", 0);
...
function NameplateLib:Update()
    ...
    for i=1, WorldFrame:GetNumChildren() do
        ...
        if ( me:GetObjectType() == NAMEPLATES_TYPE and me:GetFrameStrata() == NAMEPLATES_STRATA and me:GetFrameLevel() == NAMEPLATES_LEVEL ) then
